...
public abstract class CoroutineDispatcher : AbstractCoroutineContextElement(), ContinuationInterceptor {
    ...
    private val combiner: (CoroutineContinuation<Any>, CoroutineContextElement) -> CoroutineContinuation<Any> = { c, e ->
        if (e != this && e is ContinuationInterceptor) e.interceptContinuation(c) else c
    }

    @Suppress("UNCHECKED_CAST")
    private fun <T> combiner(): (CoroutineContinuation<T>, CoroutineContextElement) -> CoroutineContinuation<T> =
        combiner as ((CoroutineContinuation<T>, CoroutineContextElement) -> CoroutineContinuation<T>)

    public override val contextKey: CoroutineContextKey<*> = ContinuationInterceptor
